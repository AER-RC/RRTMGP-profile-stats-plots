...
for srun in "${runs[@]}"; do
  ...
  sed -i.bck "s#profiles_prefix = XYZ#profiles_prefix = profs${y_descrip}_lbl_${lblvernum}_rrtmgp_${rrtmgpvernum}_${srun}#" ${inifile}
  ...
  "${pythondir}/LBLRTM_RRTMGP_compare.py" --config_file "${inifile}" --single_stat --plot_stats --plot_profiles ${pl_bands} ${y_axis}
