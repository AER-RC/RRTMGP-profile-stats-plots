...
for ((i=0;i<${#run_base[@]};++i)); do
  ...
  sed -i.bck "s#profiles_prefix = XYZ#profiles_prefix = forcing_profs_lbl_${lblvernum}_rrtmgp_${rrtmgpvernum}_${run_forcing[$i]}-${run_base[$i]}#" ${inifile}
  ...
  "${pythondir}/LBLRTM_RRTMGP_compare.py" --config_file "${inifile}" --single_stat --plot_stats --plot_profiles --ref_forcing --band 3
